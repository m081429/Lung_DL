...
dim(input)
input[1:4,]
colnames(input)<-c("NumPatches","Status","Subject","Group")
input[1:4,]
Data[1:4,]
input$Subject = gsub(".[A-Z0-9]*","",input$Subject)
input[1:4,]
input<-read.table('final_confirm_tf_record_level2_geneexp.xls',header=F, sep="\t",stringsAsFactors=F)
colnames(input)<-c("NumPatches","Status","Subject","Group")
input[1:4,]
gsub(".tfrecords","",input$Subject)[1:4]
input$Subject = gsub(".tfrecords","",input$Subject)
input[1:4,]
data<-merge(Data,input,by="Subject")
dim(data)
data[1:4,]
data$Status<-Status.y
...
dim(val)
val
table(val$Group,val$Status)
table(test$Group,test$Status)
table(train$Group,train$Status)
...
tmp$Prediction = ifelse(tmp$Prediction > thresh, 1,0)
 tmp$Prediction
tmp$Status
tmp$Prediction<-as.integer(as.character(tmp$Prediction))
tmp$Status<-as.integer(as.character(tmp$Status))
tmp$Status
...
val$Gene
val
gene<-unique(val$Gene)[1]
tmp = val[which(val$Gene==gene),]
thresh = TRAINING_RESULTS[which(TRAINING_RESULTS$Gene==gene),'threshold']
tmp$Prediction = ifelse(tmp$Prediction > thresh, 1,0)
tmp$Prediction<-as.integer(as.character(tmp$Prediction))
tmp$Status<-as.integer(as.character(tmp$Status))
